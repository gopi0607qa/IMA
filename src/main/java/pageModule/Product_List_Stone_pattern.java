...
public class Product_List_Stone_pattern extends Testbase {
	...
	public void Product_List(ExtentTest test, WebDriver driver) throws InterruptedException, IOException, AWTException {
		...
		fun.Click(driver, Prod_Li_xp.Stone_Pattern_Page);
		...
		for(WebElement product_element: productelements) {
			...
			try {
				...
				for(String product : products) {
		            ...
		            XSSFSheet sheet = book.createSheet("Stone_pattern");
		            int rowno = 0;
		            ...
		            FileOutputStream out = new FileOutputStream(new File("C:\\eclipse\\orange_coast\\src\\test\\resources\\excel\\Stone_pattern.xlsx")); // file name with path
					book.write(out);
					out.close();
