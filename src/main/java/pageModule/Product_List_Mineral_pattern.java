...
public class Product_List_Mineral_pattern extends Testbase {
	...
	public void Product_List(ExtentTest test, WebDriver driver) throws InterruptedException, IOException, AWTException {
		...
		fun.Click(driver, Prod_Li_xp.Mineral_Pattern_Page);
		...
		for(WebElement product_element: productelements) {
			...
			try {
				...
				for(String product : products) {
		            ...
		            XSSFSheet sheet = book.createSheet("Mineral_pattern");
...
//		            row0.createCell(0).setCellValue("Product List");
		            ...
		            FileOutputStream out = new FileOutputStream(new File("C:\\eclipse\\orange_coast\\src\\test\\resources\\excel\\Mineral_pattern.xlsx")); // file name with path
					book.write(out);
					out.close();
