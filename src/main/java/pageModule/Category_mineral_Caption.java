...
public class Category_mineral_Caption extends Testbase {
	...
	public void Category(ExtentTest test, WebDriver driver, String Header) throws IOException {
		...
		fun.Click(driver, catUI_xp.Mineral_Pattern_Page);
		...
		while (catUI_xp.next_page.isDisplayed() == true) {
			...
			try {
				...
				fun.Scroll(driver, catUI_xp.Pagination_dot);
			...
			// else if(catUI_xp.Pagination_dot.isDisplayed()==false){
				...
				test.log(Status.INFO, "Page number Dot is present in Pagination",
						MediaEntityBuilder.createScreenCaptureFromPath(fun.capturescreenshot(driver)).build());
				e.printStackTrace();
	...
	public void lastpage(ExtentTest test, WebDriver driver, String Header) throws IOException {
		...
		Category_nature_Caption cap = new Category_nature_Caption();
